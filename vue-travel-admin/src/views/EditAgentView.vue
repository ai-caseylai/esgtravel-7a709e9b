...
async function loadAgentInfo()
{
    ...
    try {
        ...
        if (res.status == 200) {
            ...
            contactname.value = agentinfo.value.contactname;
            ccode.value = agentinfo.value.countrycode;
            mobile.value = agentinfo.value.mobile;
            email.value = agentinfo.value.email;
            if(agentinfo.value.status == '1')
                status.value = true;
            else
                status.value = false;
...
//console.log(contactname.value);
    const formData = new FormData();
    formData.append('ActionType', 'UpdateAgent');
  ...
  try {
      ...
      if (res.status == 200) {
        const result = res.data;
        console.log(result);
        console.log('updatestatus: ' + result.updatestatus);
...
<template>
    <div class="addnewagent" v-if="isEditMode == true">
        ...
        <form class="newform" >
            <table>
            <tr>
                <td colspan="2" style="text-align: center; justify-content: center;"><label style="color:#000;">Update Agent Information</label></td>
            </tr>
            <tr>
        ...
        <table style="border: 0px solid #ccc;">
            <tr style="border: 0px solid #ccc;">
                <td style="width:50%; text-align:right; vertical-align:middle;border: 0px solid #ccc;"><button name="Clear" @click="onClickBack" style="color: #ffffff;background:#1B78B5; width:180px; height:40px; border-radius: 12px;border: 0px solid #ced4da;">Back</button></td>
                <td style="width:50%; text-align:left; vertical-align:middle;border: 0px solid #ccc;"><button name="Submit" @click="handleSubmit" id="submit_btn"  style="color: #ffffff;background:#1B78B5; width:180px; height:40px; border-radius: 12px;border: 0px solid #ced4da;">Submit</button></td>
            </tr>
        </table>
        </form>
    </div><br>
    <div class="addnewagent" v-if="isEditMode == false">
        <table style="height: 20vh;border: 0px solid #ccc;">
            <tr style="border: 0px solid #ccc;">
                <td style="text-align: center; justify-content: center;height: 50%;border: 0px solid #ccc;"><label style="color: #1B78B5;background:#ffffff; width:250px; height:40px; border-radius: 12px;border: 0px solid #ced4da;">Record has been updated.</label><br><br></td>
            </tr>
            <tr style="border: 0px solid #ccc;">
                <td style="text-align: center; justify-content: center;height: 50%;border: 0px solid #ccc;"><button style="color: #ffffff;background:#1B78B5; width:180px; height:40px; border-radius: 12px;border: 0px solid #ced4da;" name="Submit"  @click="onClickBack">Back To Agent List</button></td>
            </tr>
        </table>
    </div>
</template>
