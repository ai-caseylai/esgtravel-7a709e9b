...
//console.log(contactname.value);
    const formData = new FormData();
    formData.append('ActionType', 'AddNewCompanyInfo');
...
<template>
    <div class="modifycompany" v-if="isEditMode == true">
        ...
        <form class="newform">
            <table>
            <tr>
                <td colspan="2" style="text-align: center; justify-content: center;"><label style="color:#1B78B5;">Modify Company Information</label></td>
            </tr>
            <tr>
    ...
    <div class="modifycompany" v-if="isEditMode == false">
        <table style="height: 100%;border: 0px solid #ccc;">
            ...
            <tr style="border: 0px solid #ccc;">
                <td style="text-align: center; justify-content: center;height: 50%;border: 0px solid #ccc;"><button style="background:#1B78B5; width:180px; height:40px; border-radius: 12px;border: 0px solid #ced4da;color: #fff;" name="Submit"  @click="onClickAddNew">Add New Company</button></td>
            </tr>
        </table>
    </div>
</template>
